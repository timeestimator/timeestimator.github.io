...
function init() {
	...
	for (var i = dotplots.length - 1; i >= 0; i--) {
		var div = tmpl("dotplot_template", {id: dotplots[i]});
		iH += div;
	}
	document.getElementById("gallery1").innerHTML = iH;

	iH = "";
	for (var i = 0; i < nb_linecharts; i++) {
		var div = tmpl("linechart_template", {id: i+1});
		iH += div;
	}
	document.getElementById("gallery2").innerHTML = iH;
...
function toggleGallery2() {
	var container = document.getElementById("res-linecharts-container");
	container.hidden = !container.hidden;
	if (container.hidden) {
		document.getElementById("arrow3").src = "images/arrow-down.png";
	} else {
